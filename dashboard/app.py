...
gen_melted = gen_df.melt(id_vars="year", var_name="source", value_name="generation_twh")
gen_melted["source"] = (
    gen_melted["source"]
    .str.replace("gen_", "")
    .str.replace("_twh", "")
...
share_melted = share_df.melt(id_vars="year", var_name="source", value_name="share_pct")
share_melted["source"] = (
    share_melted["source"]
    .str.replace("share_", "")
    .str.replace("_pct", "")
...
fig4 = px.line(
    df.dropna(subset=["demand_twh", "primary_energy_twh"]),
    x="year",
    y=["demand_twh", "primary_energy_twh"],
    labels={"value": "TWh", "year": "Year", "variable": "Metric"},
)
st.plotly_chart(fig4, use_container_width=True)
st.caption(
    "Primary energy consumption includes all energy forms (gas, oil, coal) used across transport, industry, and power generation. Electricity demand is only the subset that flows through the grid. The gap reflects energy lost in conversion and fuel burned directly outside the power sector."
)
